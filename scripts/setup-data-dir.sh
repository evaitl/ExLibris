...
for script in "$ROOT/scan_books.py" "$ROOT/update_epubs.py" "$ROOT/manage_users.py" "$ROOT/cleanup_library.py" "$ROOT/serve_web.py" "$ROOT/convert_epub2.py" "$ROOT/scripts/shard-covers.py" "$ROOT/scripts/convert-epubs-in-dir.py" "$ROOT"/scripts/*.sh "$ROOT"/web/cgi-bin/*.py; do
